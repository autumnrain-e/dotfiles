...
# Aliases Python — macOS has python3, not a `pip` command.
...
# Aliases AeroSpace
# Mirrors the AeroSpace block in fish/config.fish — see the comments there for why
# asrestart has to kill all three processes. Only the background syntax differs:
# zsh takes `& disown`, fish needs `&; disown`.
