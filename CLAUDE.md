...
- **Logo chip** (`items/spaces.sh`): left-edge chip, one static glyph — `$ICON_LOGO` (`nf-md-coffee`, **U+F0176**) in `$YELLOW` on `$GROUP_BG`, `icon.padding_left/right=10`, no script and no `update_freq`. Width is **dynamic on purpose**: a single fixed glyph never re-measures, and a pinned width would swallow the item's own `padding_right`. `padding_right` stays 0 — the 10px gap to workspace 1 is `logo_separator`'s `SPACE_EDGE=8` plus the chip's `SPACE_GAP=2`, matching the app-chip end of the row. Swap the glyph by uncommenting one of the alt lines under `ICON_LOGO` in `icons.sh` (`nf-pom-away` U+E007, `nf-md-ghost` U+F02A0 — the original logo).
  - It replaced the **doom face-cycle animation** (removed 2026-08-24 at the user's request): `plugins/doom.sh`, `assets/doom/`, `assets/doom.png`, `assets/doom-src.png` and `scripts/prepare-doom-faces.py` are all deleted, so `assets/` and `scripts/` no longer exist in the package. Recover any of it from git history (`git log --diff-filter=D -- sketchybar/.config/sketchybar/plugins/doom.sh`); the unprocessed HUD source frames were never in the repo, they live in `~/Downloads/doom_faces`.
