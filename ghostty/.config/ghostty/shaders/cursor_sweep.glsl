// ============================================================================
// Cursor Sweep — an animated trail that shrinks from the previous cursor
// position to the current one. This is the Ghostty replacement for kitty's
// built-in `cursor_trail`, which Ghostty has no native equivalent for.
//
// Vendored from https://github.com/sahaj-b/ghostty-cursor-shaders (MIT),
// file `cursor_sweep.glsl` @ 0a274beac8b9 (2026-06-16). Kept as a file in this
// repo rather than a `git clone` into ~/.config/ghostty/shaders, because that
// directory is a stow symlink into this repo and a nested clone there would
// both shadow the package and land a repo-inside-a-repo.
//
// LOCAL CHANGES vs upstream (keep this list current if you re-sync):
//   1. This header.
//   2. TRAIL_COLOR pinned to #ddc7a1 instead of following the cursor color.
// Everything below that is byte-for-byte upstream, including the commented-out
// easing functions — swap which `ease()` is uncommented to change the curve.
//
// Ghostty reloads shaders with the rest of the config (cmd+shift+,). A shader
// that fails to COMPILE is silently ignored: the error never surfaces as a
// config error, only in the render-thread log. So "no trail" after an edit
// means a compile error, not a missing setting.
// ============================================================================
...
// --- CONFIGURATION ---
// LOCAL CHANGE: #ddc7a1 — the gruvbox-material foreground tone this repo also
// uses for JankyBorders' `inactive_color`, and a touch brighter than the
// theme's own foreground #d4be98. Deliberately NOT the cursor color (#7daea3,
// aqua) that upstream defaults to. Values are 0-255 over 255 so the hex stays
// readable; sRGBToLinear is required or the trail renders too bright.
vec4 TRAIL_COLOR = vec4(sRGBToLinear(vec3(221.0, 199.0, 161.0) / 255.0), 1.0);
// Upstream default (follow the cursor color), for revert:
